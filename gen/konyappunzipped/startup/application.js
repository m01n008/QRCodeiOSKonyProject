...
function openServiceDefCall(req_url, inputParam, serviceID, objectID, callBack) {
    let request = new kony.net.HttpRequest();
    let method = inputParam.options.CRUD_TYPE;
    let data = inputParam.options.data || {};
    data.pathParam = data.pathParam || {};
    data.queryParam = data.queryParam || {};
    let reqBody = {};
    let httpHeader = inputParam.httpheaders || {};
    if (Object.keys(data.pathParam).length) {
        req_url = replacePathParam(req_url, data.pathParam);
    }
    if (Object.keys(data.queryParam).length) {
        req_url = appendQueryParam(req_url, data.queryParam);
    }
    reqBody = makeFormData(data, reqBody);
    kony.print("Invoking Open API service URL " + req_url);
    request.open(method, req_url, true);
    ...
    request.onReadyStateChange = (function() {
        if (request.readyState == 4) {
            if (request.status == 200) {
                callBack(request.status, parseKonyNetHttpResponse(request));
            } else {
...
var parseKonyNetHttpResponse = function(httpRequest) {
    var parsedResponse = {};
    ...
    if (isNullOrUndefined(httpRequest) || isNullOrUndefined(httpRequest.response)) {
        kony.print("parseHttpResponse :: Null or Invalid response received");
    } else if (httpRequest.responseType && httpRequest.responseType === "blob") {
        parsedResponse = {
            "isRawResponse": true,
            "rawResponse": httpRequest.response
        };
    } else {
        //Defaulting to JSON format
        if (isJsonObject(httpRequest.response)) {
            parsedResponse = httpRequest.response;
        } else {
            try {
                parsedResponse = JSON.parse(httpRequest.response);
            } catch (jsonError) {
                kony.print("parseHttpResponse :: Response is not JSON String");
                parsedResponse = {
                    "isRawResponse": true,
                    "rawResponse": httpRequest.response
                };
...
var replacePathParam = function(req_url, pathParam) {
    for (let key in pathParam) {
        let pathParamStr = "{" + key + "}";
        if (req_url.indexOf(pathParamStr)) {
            req_url = req_url.replace(pathParamStr, pathParam[key]);
        }
    }
    return req_url;
};
var appendQueryParam = function(req_url, queryParam) {
    let cnt = 0;
    for (let key in queryParam) {
        if (cnt == 0) {
            req_url = req_url.concat("?", `${key}=${queryParam[key]}`);
        } else {
            req_url = req_url.concat("&", `${key}=${queryParam[key]}`);
        }
        cnt++;
...
var makeFormData = function(data, reqBody) {
    for (let eachData in data) {
        if (eachData != 'queryParam' && eachData != 'pathParam') {
            reqBody[eachData] = data[eachData];
        }
    }
