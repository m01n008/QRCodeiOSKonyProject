...
(function() {
    ...
    DataModel_QueryBuilder = function() {
        ...
        QueryBuilder.prototype.buildSelectList = function(baseModel, config) {
            ...
            for (var key in baseModel.relations) {
                ...
                var modeldefination = kony.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition(modelstr, config.serviceName);
                var configdefination = kony.mvc.MDAApplication.getSharedInstance().modelStore.getConfig(modelstr, config.serviceName);
    ...
    DataModel_DBAssembler = function(Error) {
        ...
        assembler.fromDBJson = function(BaseModelClass, config, dbJson) {
            ...
            for (var attribute in attributeMap) {
                ...
                if (model) {
                    var modelDefinition = kony.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition(model, config.serviceName);
                    var modelConfig = kony.mvc.MDAApplication.getSharedInstance().modelStore.getConfig(model, config.serviceName);
    ...
    DataModel_RepositoryManager = function(BaseRepository) {
        ...
        /**
         * @function _isNewTemplate will check whether the template is new or old
         * @return {boolean}
         * @private
         */
        RepositoryManager.prototype._isNewTemplate = function() {
            if (this.repoConfig['version'] === 1.1) {
                return true;
            } else {
                return false;
            }
        ...
        /**
         * @function _findObjectServiceName will search for object service name using the object
         * @param modelName
         * @return {string} Object Service Name
         * @private
         */
        RepositoryManager.prototype._findObjectServiceName = function(modelName) {
            for (objSvc in this.repoConfig) {
                objModel = this.repoConfig[objSvc][modelName];
                if (!kony.model.Util.isNullOrUndefined(objModel)) {
                    var objSvcName = objSvc;
                    break;
                }
            }
            return objSvcName;
        ...
        /**
         * @function _findModelInRepoMap will check for object in repoMap
         * @param modelName
         * @return {object} Object map
         * @private
         */
        RepositoryManager.prototype._findModelInRepoMap = function(modelName) {
            var repo;
            for (objSvc in this.repoMap) {
                repo = this.repoMap[objSvc][modelName];
                if (!kony.model.Util.isNullOrUndefined(repo)) {
                    break;
                }
            }
            return repo;
        ...
        /**
         * @function _fetchObjectServiceName will check for object service name
         * @param modelName
         * @return {string} objSvcName or else will throw an error
         * @private
         */
        RepositoryManager.prototype._fetchObjectServiceName = function(modelName) {
            var objSvcName = this._findObjectServiceName(modelName);
            if (kony.model.Util.isNullOrUndefined(objSvcName)) {
                throw Error(modelName + ' object is not present in Object service');
            }
            return objSvcName;
        };
        /**
         * @function _setModelRepository will set the parameters of model for setRepository
         * @param model
         * @param modelConfig
         * @param objSvcName
         * @private
         */
        RepositoryManager.prototype._setModelRepository = function(model, modelConfig, objSvcName) {
            ...
            this.setRepository(model, modelPath, configPath, repository, objSvcName);
        };
        /**
         ...
         */
        RepositoryManager.prototype.getRepository = function(model, objSvcName) {
            var repo;
            if (!this._isNewTemplate()) {
                repo = this.repoMap[model];
                if (kony.model.Util.isNullOrUndefined(repo)) {
                    var modelConfig = this.repoConfig[model];
                    this._setModelRepository(model, modelConfig);
                }
                return this.repoMap[model];
            } else {
                if (kony.model.Util.isNullOrUndefined(objSvcName)) {
                    repo = this._findModelInRepoMap(model);
                    if (repo) {
                        objSvcName = repo.config.serviceName;
                    }
                } else {
                    if (!kony.model.Util.isNullOrUndefined(this.repoMap[objSvcName])) repo = this.repoMap[objSvcName][model];
                }
                if (kony.model.Util.isNullOrUndefined(repo)) {
                    if (kony.model.Util.isNullOrUndefined(objSvcName)) {
                        objSvcName = this._fetchObjectServiceName(model);
                    }
                    var modelConfig = this.repoConfig[objSvcName][model];
                    this._setModelRepository(model, modelConfig, objSvcName);
                }
                return this.repoMap[objSvcName][model];
            }
        };
        /**
        ...
        RepositoryManager.prototype.getAllRepository = function() {
            var repositories = [];
            if (this._isNewTemplate()) {
                for (var objSvc in this.repoMap) {
                    for (var model in this.repoMap[objSvc]) {
                        repositories.push(this.repoMap[objSvc][model]);
                    }
                }
            } else {
         ...
         * @return 
         */
        RepositoryManager.prototype.setRepository = function(modelName, modelPath, configPath, ModelRepository, objSvcName) {
            ...
            if (!this._isNewTemplate()) {
                ...
                if (kony.model.Util.isNullOrUndefined(this.repoMap[objSvcName])) this.repoMap[objSvcName] = {};
                this.repoMap[objSvcName][modelName] = repository;
            }
        };
    ...
    commonUtils_MDAApplication = function(RepositoryManager, MDAModuleManager, UIBinderBuilder) {
        ...
        MDAApplication.prototype = {
            initialize: function() {
                ...
                var scope = this;
                this.modelStore = {
                    getModelDefinition: function(modelName, objSvcName) {
                        if (!scope.repositoryManager._isNewTemplate()) {
                            var modelConfig = scope.repositoryManager.repoConfig[modelName];
                        } else {
                            if (kony.model.Util.isNullOrUndefined(objSvcName)) {
                                objSvcName = scope.repositoryManager._fetchObjectServiceName(modelName);
                            }
                            var modelConfig = scope.repositoryManager.repoConfig[objSvcName][modelName];
                        }
                        var modelPath = modelConfig['model'];
                        var model = require(modelPath);
                        return model;
                    },
                    getConfig: function(modelName, objSvcName) {
                        if (!scope.repositoryManager._isNewTemplate()) {
                            var modelConfig = scope.repositoryManager.repoConfig[modelName];
                        } else {
                            if (kony.model.Util.isNullOrUndefined(objSvcName)) {
    ...
    DataModel_BaseModel = function(ModelRelation) {
        ...
        BaseModel.prototype.save = function(onCompletion, accessMode, options) {
            ...
            var objSvc = this.objServiceName;
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).save(this, onCompletion, accessMode, options);
        };
        /**
         * @function getByPrimaryKey  fetches data for the specified model based on the primary keys
         * @param  {String/JSON} primaryKey   primitive in case of one primary key and json in case of composite primary key
        ...
        BaseModel.getByPrimaryKey = function(PrimaryKey, onCompletion, accessMode, options) {
            ...
            var objSvc = this.prototype.objServiceName;
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).getByPrimaryKey(PrimaryKey, onCompletion, accessMode, options);
        };
        /**
         * @function getByPrimaryKey  fetches data for the specified model based on the primary keys
         * @param  {String/JSON} primaryKey   primitive in case of one primary key and json in case of composite primary key
        ...
        BaseModel.prototype.update = function(onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).update(this, onCompletion, accessMode, options);
        };
        /**
         * @function partialUpdate partially update the model data to the backend
         * @param  {function} onCompletion success callback
        ...
        BaseModel.prototype.partialUpdate = function(onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).partialUpdate(this, onCompletion, accessMode, options);
        };
        /**
         * @function remove deletes the specified model from the backend
         * @param  {function} onCompletion success callback
        ...
        BaseModel.prototype.remove = function(onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).remove(this, onCompletion, accessMode, options);
        };
        /**
        ...
        BaseModel.removeById = function(primaryKey, onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).removeById(this, primaryKey, onCompletion, accessMode, options);
        };
        /**
         * @function removeById deletes the specified model from the backend with a specific id
         * @param {JSON/STRING} primaryKey primary key value or json representing the key and value
        ...
        BaseModel.getAll = function(onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).getAll(onCompletion, accessMode, options);
        };
        /**
         * @function getAll  fetches  all the data for the specified model
         * @param  {type} onCompletion success callback
        ...
        BaseModel.customVerb = function(customVerb, params, onCompletion, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).customVerb(customVerb, params, onCompletion, options);
        };
        /**
         * @function customVerb fetches data for the specified model using specified customVerb
         * @param  {String} customVerb   name of the custom verb to be executed
        ...
        BaseModel.getByCriteria = function(criteria, onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model, objSvc).getByCriteria(criteria, onCompletion, accessMode, options);
        };
        /**
         * @function getByCriteria  fetches data for the specified model based on the criteria passed
         * @param  {Json} criteria        criteria object based on which query is build in case of online and offline
