 /*
  * kony-sdk-ide Version 202404.0.0
  */
 /**
  * Kony namespace
  * @namespace kony
 ...
 kony.sdk = function() {
     ...
     this.customAuthorizationHeadersCallback = null;
     this.customAuthorizationHeaders = {};
     var userId = "";
     ...
     this.setCustomAuthorizationHeaderCallback = function(callback) {
         this.customAuthorizationHeadersCallback = callback;
     };
     this.appendCustomAuthorizationHeaders = function(headers) {
         kony.sdk.logsdk.perf("Executing appendCustomAuthorizationHeaders");
         var customAuthorizationHeaders = currentObj.customAuthorizationHeaders;
         if (!kony.sdk.isNullOrUndefined(customAuthorizationHeaders)) {
             if (kony.sdk.isNullOrUndefined(headers)) {
                 headers = {};
             }
             for (var obj in customAuthorizationHeaders) {
                 if (kony.sdk.isNullOrUndefined(headers[obj])) {
                     headers[obj] = customAuthorizationHeaders[obj];
                 }
             }
         }
         kony.sdk.logsdk.perf("Executing Finished appendCustomAuthorizationHeaders");
     };
 ...
 kony.sdk.claimsRefresh = function(callback, failureCallback) {
     ...
     if (typeof(konyRef.customAuthorizationHeadersCallback) === 'function') {
         konyRef.customAuthorizationHeadersCallback(function(customHeaders) {
             konyRef.customAuthorizationHeaders = customHeaders;
             _claimRefresh(callback, failureCallback);
         });
     } else {
         _claimRefresh(callback, failureCallback);
     }
 ...
 function _claimRefresh(callback, failureCallback) {
 ...
 function getLatestServiceDocIfAvailable(data, callback) {
     ...
     if (!kony.sdk.isNullOrUndefined(serverETag) && (kony.sdk.isNullOrUndefined(currentETag) || currentETag != serverETag)) {
         ...
         networkProvider.post(_serviceUrl, null, headers, function(successResponse) {
             //Cache latest metadata(appKey, appSecret and serviceDoc) in DS and update integrity
             //check for next app launch, if the recieved server response is valid.
             if (isFabricAppConfigResponseValid(successResponse)) {
 ...
 function isFabricAppConfigResponseValid(response) {
     if (kony.sdk.util.isNullOrUndefinedOrEmptyObject(response)) {
         kony.sdk.logsdk.warn("Invalid Service doc: " + JSON.stringify(response));
         return false;
     }
     if (kony.sdk.util.isNullOrUndefinedOrEmptyObject(response.baseId) || kony.sdk.util.isNullOrUndefinedOrEmptyObject(response.name)) {
         kony.sdk.logsdk.warn("Invalid Service doc, either mf_app_name: " + JSON.stringify(response.name) + "  OR mf_base_id: " + JSON.stringify(response.baseId) + " is missing in response");
         return false;
     }
     if (kony.sdk.util.isNullOrUndefinedOrEmptyObject(response.reportingsvc) || kony.sdk.util.isNullOrUndefinedOrEmptyObject(response.reportingsvc.custom) || kony.sdk.util.isNullOrUndefinedOrEmptyObject(response.reportingsvc.session)) {
         kony.sdk.logsdk.warn("Invalid Service doc,  reporting Service url(s) missing: " + JSON.stringify(response.reportingsvc));
         return false;
     }
     return true;
 }
 ...
 kony.sdk.constants = {
     ...
     DEEPLINK_VALID_OAUTH_SESSIONID: "oauth_session_id",
     ...
     OAUTH_LOGOUT_WINDOW_TIMEOUT: "oauthLogoutPopupTimeout",
  ...
  */
 kony.sdk.completeSingleWindowLogin = function(code, oauthSessionId, loginCompleteCallback) {
         ...
         kony.application.removeQueryParamsByKey(kony.sdk.constants.DEEPLINK_VALID_OAUTH_SESSIONID);
         ...
         function performTokenCall(networkResponseJSON) {
             ...
             if (!kony.sdk.isNullOrUndefined(oauthSessionId)) {
                 bodyParams[kony.sdk.constants.DEEPLINK_VALID_OAUTH_SESSIONID] = oauthSessionId;
             }
     ...
     this.login = function(options, successCallback, failureCallback) {
         ...
         function extractOauthOptions(authOptions) {
             ...
             if (kony.sdk.getPlatformName() === kony.sdk.constants.PLATFORM_THIN_CLIENT) {
                 if (!kony.sdk.isNullOrUndefined(authOptions[kony.sdk.constants.LOGIN_OPTIONS])) {
                     _oauthOptions[kony.sdk.constants.COOKIES_DISABLED] = authOptions[kony.sdk.constants.LOGIN_OPTIONS][kony.sdk.constants.COOKIES_DISABLED];
                 }
                 if (isSingleWindowLoginEnabled === true) {
     ...
     function _logout(successCallback, failureCallback, options) {
         ...
         function extractLogoutOptions(logoutOptions) {
             if (!kony.sdk.util.isNullOrUndefinedOrEmptyObject(logoutOptions)) {
                 ...
                 if (logoutOptions.hasOwnProperty("oauthLogoutPopupTimeout") && "number" === typeof(logoutOptions.oauthLogoutPopupTimeout)) {
                     if (logoutOptions.oauthLogoutPopupTimeout > 3) {
                         oauthLogoutTimer = logoutOptions.oauthLogoutPopupTimeout;
                     }
                 }
             }
         }
         ...
         if (!isLoggedIn()) {
             ...
             if (!kony.sdk.isOAuthLogoutInProgress) {
                 ...
                 oauthOptions[kony.sdk.constants.OAUTH_LOGOUT_WINDOW_TIMEOUT] = oauthLogoutTimer;
                 if (!kony.sdk.isNullOrUndefined(customOAuthParams)) {
                     oauthOptions[kony.sdk.constants.CUSTOM_QUERY_PARAMS_FOR_OAUTH] = customOAuthParams;
                 }
 ...
 */
 ...
 function IntegrationService(konyRef, serviceName) {
     ...
     function _invokeOperation(operationName, headers, data, isRetryNeeded, successCallback, failureCallback, options) {
         ...
         var kContentType = null;
         if (options && options.hasOwnProperty('passthrough') && options["passthrough"] === true) {
         ...
         networkProvider.post(serviceUrl + "/" + operationName, dataToSend, defaultHeaders, networkSuccessCallback, networkFailureCallback, kContentType, options);
     }
             ...
             if (data.geofenceID !== "refreshBoundary") {
                 ...
                 if (action && action.clientAction === "notifyEngagementServer") {
                     ...
                     try {
                         ...
                         var dateString = new Date(date).toString().slice(4, 24) + " " + new Date().toString().match(/([-\+][0-9]+)\s/)[1];
 ...
 function OAuthHandler(serviceUrl, providerName, appkey, callback, type, options, isMFVersionCompatible) {
     ...
     if (typeof(XMLHttpRequest) !== 'undefined') {
         ...
         if (isLogout) {
             ...
             kony.timer.schedule("SPALogout", handleLogoutInSPA, options.oauthLogoutPopupTimeout, false);
         } else {
             ...
             if (!kony.sdk.isNullOrUndefined(kony.sdk.pkceUtilityInstance)) {
                 if (kony.sdk.util.isJsonObject(options) && options[kony.sdk.constants.NO_POP_UP] === true && options[kony.sdk.constants.COOKIES_DISABLED] === true) {
                     requestUrl = kony.sdk.pkceUtilityInstance.appendAppChallengeOnURL(requestUrl, true);
                 } else {
 ...
 kony.setupsdks = function(initConfig, successCallBack, errorCallBack) {
     ...
     function callAnonymousLoginIfRequired(sdkObj) {
         if (kony.sdk.skipAnonymousCall || kony.licensevar.isInteractive === false) {
             // Avoid calling anonymous login if the app is launched in Background (i.e. Scheduled background services or Broadcast Reciever) 
             // OR app developer has specifically disabled anonymous login call. (i.e. when all Fabric services are public)
             // Enabling isAnonymousProvider flag to postpone the anonymous login for any protected service.
 ...
 */
 ...
 function konyNetworkProvider() {
     this.post = function(url, params, headers, successCallback, failureCallback, konyContentType, options) {
         ...
         if (!kony.sdk.isNullOrUndefined(kony.sdk.currentInstance)) {
             ...
             kony.sdk.currentInstance.appendCustomAuthorizationHeaders(headers);
         }
 ...
 kony.sdk.util.getUtilityForPKCE = (function() {
     ...
     function helperClassPKCE() {
          ...
          */
         this.appendAppChallengeOnURL = function(url, forceAppChallengeAsQueryForThinClient) {
             if (!kony.sdk.isNullOrUndefined(secureParams) && !kony.sdk.isNullOrUndefined(url)) {
                 ...
                 if (kony.sdk.getPlatformName() === kony.sdk.constants.PLATFORM_THIN_CLIENT) {
                     if (forceAppChallengeAsQueryForThinClient === true) {
