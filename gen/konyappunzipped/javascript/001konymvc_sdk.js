...
();
_kony.mvc.ModelController = function () {
...
kony.mvc.isControllerAvailable = function (formFriendlyName, appName) {
    var ctrlName = null,
    tmpFormName = null,
...
_kony.mvc.executeInJsContext = function (templateView, functionName, subargs) {
    var appName = templateView._appName ? templateView._appName : _kony.mvc.getCurrentAppName();
    var ctrlName2ControllerMap = _kony.mvc.getControllerName2ControllerMap(appName);
...
_kony.mvc.loadAllExtensionControllers = function (tmpController, config, friendlyName) {
    ...
    if (null != tmpControllerName2) {
        if (typeof tmpControllerName2 === "string") {
            ...
            config2 = kony.utils.LoadJSFile(uwPath + "/" + tmpControllerName2);
...
_kony.mvc.CreateMasterWidgetController = function (userWidgetName, uwInstanceName, args, hasMultipleViews) {
    var appName = _kony.mvc.getCurrentAppName();
    if (args[0]) {
    ...
    if (null != formID) {
        ...
        if (hasMultipleViews)
            uwPath = uwPath.substring(0, uwPath.lastIndexOf("/"));
...
_kony.mvc.initializeViewController = function (viewController, modelController) {
    if (modelController) {
...
_kony.mvc.initializeModelController = function (modelController) {
...
_kony.mvc.initializeMasterController = function (userWidgetName, uwInstanceName, args) {
    ...
    if (typeof kony.mvc.registry.get(userWidgetName, appName) == "object") {
        var modelController = _kony.mvc.createModelController(userWidgetName, appName);
        var viewName;
        ...
        if (viewName == null || viewName == undefined) {
            kony.print("Undefined/Null viewName");
            return
        }
        var viewController = _kony.mvc.CreateMasterWidgetController(userWidgetName + "." + viewName, uwInstanceName, args, true);
        if (!viewController) {
            kony.print("Undefined/Null _kony.mvc.CreateMasterWidgetController() value");
            return
        }
        _kony.mvc.initializeViewController(viewController, modelController);
        _kony.mvc.initializeModelController(modelController);
        return viewController.view
    } else {
        var tmpController = _kony.mvc.CreateMasterWidgetController(userWidgetName, uwInstanceName, args, false);
        return tmpController.view
    }
};
_kony.mvc.changeView = function (userWidgetInstance, userWidgetName, args, newViewType) {
    var uwAppName = args[0] ? args[0].appName : _kony.mvc.getCurrentAppName();
    var viewId2ControllerNameMap = _kony.mvc.getViewId2ControllerNameMap(uwAppName);
    var cntrlName2ControllerMap = _kony.mvc.getControllerName2ControllerMap(uwAppName);
    var uwControllerName = viewId2ControllerNameMap[userWidgetInstance.id];
    var uwController = cntrlName2ControllerMap[uwControllerName];
    var modelController = uwController.controller;
    var newViewController = _kony.mvc.CreateMasterWidgetController(userWidgetName + "." + newViewType, userWidgetInstance.id, args, true);
    _kony.mvc.initializeViewController(newViewController, modelController);
    userWidgetInstance._konyInitialized = false;
    userWidgetInstance._konyControllerName = newViewController.view._konyControllerName;
    _kony.mvc.setMasterContract(userWidgetInstance);
...
_kony.mvc.createModelController = function (baseUserWidgetName, appName) {
    var modelControllerName = kony.mvc.registry.getControllerName(baseUserWidgetName, appName);
    if (!modelControllerName) {
        kony.print("Undefined/Null Controller Name in registry");
        return
    }
    var uwPath = baseUserWidgetName.replace(/\./g, "/");
    var config = kony.utils.LoadJSFile(uwPath + "/" + modelControllerName);
    if (!config) {
        kony.print("Requiring Model controller failed.");
        return
    }
    var modelController = new _kony.mvc.ModelController(baseUserWidgetName);
    modelController._baseUserWidgetName = baseUserWidgetName;
    modelController.Name = modelControllerName + "_" + (++_konyControllerCounter).toString();
    modelController.appName = appName;
    _kony.mvc.ctrlname2ControllerMap[modelController.Name] = modelController;
    _kony.mvc.loadAllExtensionControllers(modelController, config, baseUserWidgetName);
    return modelController
};
