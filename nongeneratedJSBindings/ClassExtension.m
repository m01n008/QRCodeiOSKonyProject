...
extern JSValue* extractStructArgument(const char* type, NSInvocation* invocation, NSUInteger index, JSContext* context);
extern void setStructReturnValue(const char* type, JSValue* value, NSInvocation* invocation);
...
+(id) createClass: (NSString *) className extending:(NSString *)baseClass conformingTo:(NSArray *)protocols withInstanceMembers:(JSValue *)instanceMembers
{
    ...
    if(baseCls == nil)
        return baseCls;//will be returned as undefined in JS if the base class is not available
...
+(id)import: (NSString*) className
{
    ...
    if(cls == nil)//will be return as undefined in JS if the class is not available
        return cls;
